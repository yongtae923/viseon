...
def _resolve_path(path_value):
    """Resolve path values against project root."""
...
def _resolve_cfg_paths(cfg):
    """Normalize known config path fields after loading yaml."""
    path_keys = [
        "save_path",
        "data_directory",
        "base_config",
...
def _format_duration(seconds):
    """Format seconds as 'Xh Ym Zs' or 'Xm Ys' or 'Xs'."""
    s = int(seconds)
    if s >= 3600:
...
def train(dataset, models, training_pipeline, logging, cfg):
    ...
    epoch = 0
    print_epoch_timing = cfg.get('print_epoch_timing', False)
    log_stimulation_histogram = cfg.get('log_stimulation_histogram', True)
    t_start = time.time() if print_epoch_timing else None

    while epoch <= cfg['epochs'] and not_improved_count < cfg['early_stop_criterium']:
        epoch_start = time.time() if print_epoch_timing else None
        total_epochs = cfg['epochs'] + 1
                ...
                for key in cfg['save_output']:
                    ...
                    if len(shape) == 4:  # Image batch (N,C,H,W)
                        ...
                        tb_writer.add_images(key,
                                             ...
                                             timestamp['samples'], dataformats='NCHW')
                    elif len(shape) == 2 and log_stimulation_histogram: # (N, P)
        ...
        if print_epoch_timing and epoch_start is not None:
            elapsed_total = time.time() - t_start
            completed = epoch + 1
            avg_epoch_time = elapsed_total / completed
            remaining_epochs = max(0, cfg['epochs'] - epoch)
            eta = remaining_epochs * avg_epoch_time
            print(f"  Epoch {epoch}/{cfg['epochs']} done. Elapsed: {_format_duration(elapsed_total)} | Est. remaining: {_format_duration(eta)}")
        epoch += 1
...
def main(args):
    ...
    cfg = _resolve_cfg_paths(load_config(config_path))
